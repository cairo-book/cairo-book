...
impl ArrayFilterExt of ArrayFilterExtTrait {
    // Needed in Cairo 2.9.2 because of a bug in inlining analysis.
    #[inline(never)]
    fn filter<T, +Copy<T>, +Drop<T>, F, +Drop<F>, impl func: core::ops::Fn<F, (T,)>[Output: bool], +Drop<func::Output>>(
        self: Array<T>, f: F,
    ) -> Array<T> {
...
fn main() {
    ...
    //ANCHOR: map_usage
    let _double = array![1, 2, 3].map(|item: u32| item * 2);
    let _another = array![1, 2, 3].map(|item: u32| {
        let x: u64 = item.into();
        x * x
    });
    ...
    //ANCHOR: filter_usage
    let _even = array![3, 4, 5, 6].filter(|item: u32| item % 2 == 0);
