...
To show Cairo where to find an item in a module tree, we use a path in the same
...
{{#ref front_of_house}} for the restaurant we used in the last chapter. We have
a crate named _restaurant_ in which we have a module named `front_of_house` that
...
{{#label path-types}} <span class="caption">Listing {{#ref path-types}}: Calling
the `add_to_waitlist` function using absolute and relative paths</span>
...
the ["Exposing Paths with the `pub` Keyword"][pub] section.
...
_/front_of_house/hosting/add_to_waitlist_ to run the _add_to_waitlist_ program;
...
be using the path _./front_of_house/hosting/add_to_waitlist_. Starting with a
module name means that the path is relative to the current module.

Let’s try to compile Listing {{#ref path-types}} and find out why it won’t
compile yet! We get the following error:
...
[pub]:
  ./ch07-03-paths-for-referring-to-an-item-in-the-module-tree.md#exposing-paths-with-the-pub-keyword
...
shown in Listing {{#ref pub-keyword-not-compiling}}.
...
{{#label pub-keyword-not-compiling}} <span class="caption">Listing
{{#ref pub-keyword-not-compiling}}: Declaring the `hosting` module as `pub` to
use it from `eat_at_restaurant`</span>

Unfortunately, the code in Listing {{#ref pub-keyword-not-compiling}} still
results in an error.
...
keyword before its definition, as in Listing {{#ref pub-keyword}}.
...
{{#label pub-keyword}} <span class="caption">Listing {{#ref pub-keyword}}:
...
Consider the code in Listing {{#ref relative-path}} that models the situation in
...
{{#label relative-path}} <span class="caption">Listing {{#ref relative-path}}:
Calling a function using a relative path starting with `super`</span>
