...
program in Listing {{#ref config_max}} that matches on an `Some<u8>` value in
...
{{#label config_max}} <span class="caption">Listing {{#ref config_max}}: A
`match` that only cares about executing code when the value is `Some`</span>
...
behaves the same as the `match` in Listing {{#ref config_max}}:
...
{{#ref match-pattern-bind}}, where the `Quarter` variant also held a `UsState`
...
> Note: `else` is not yet supported for let chain expressions; this will be
