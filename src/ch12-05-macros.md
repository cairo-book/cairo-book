...
of features in Cairo: _declarative_ macros with `macro` and three kinds of
_procedural_ macros covered in
...
known as _metaprogramming_. In Appendix C, we discuss derivable traits and the
...
The most simple form of macros in Cairo is the _declarative macro_, also
...
[Chapter 6](./ch06-00-enums-and-pattern-matching.md), `match` expressions are
...
Let’s walk through the pattern pieces in the example above:
...
> `Ctrl+Shift+P` and then `Cairo: Recursively expand macros for item at caret`.
...
- Item-producing macros (structs, enums, functions, etc.) are not yet supported;
  support will be added in future versions.
- For attributes, derives, and crate-wide transformations, prefer procedural
  macros (next section).
