...
[Chapter {{#chap enums-and-pattern-matching}}][enums]), and their first
...
function and passed `rect1` as an argument, we can instead use _method syntax_
...
> types in a same trait (mixing `Rectangle` and `Circle` methods, for example).
> But **this is not a recommended practice** as it can lead to confusion.
...
If you are familiar with Rust, you may find Cairo's approach confusing because
methods cannot be defined directly on types. Instead, you must define a
[trait](./ch08-02-traits-in-cairo.md) and an implementation of this trait
...
We call _associated functions_ all functions that are defined inside an `impl`
...
the following code is equivalent to the code shown in the _Methods with several
parameters_ section, which has each method in its own `trait` and `impl` blocks.
