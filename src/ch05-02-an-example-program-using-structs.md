...
rectangle. Listing {{#ref area-fn}} shows a short program with one way of doing
...
{{#label area-fn}} <span class="caption">Listing {{#ref area-fn}}: Calculating
the area of a rectangle specified by separate width and height variables.</span>
...
that in the
[Tuple Section of Chapter {{#chap common-programming-concepts}}](./ch02-02-data-types.md#the-tuple-type).

## Refactoring with Tuples

Listing {{#ref rectangle-tuple}} shows another version of our program that uses
tuples.
...
{{#label rectangle-tuple}} <span class="caption">Listing
{{#ref rectangle-tuple}}: Specifying the width and height of the rectangle with
a tuple.</span>
...
{{#label rectangle-struct}} <span class="caption">Listing
{{#ref rectangle-struct}}: Defining a `Rectangle` struct.</span>
...
We've already described how to perform type conversion on in-built types, see
[Data Types > Type Conversion][type-conversion]. In this section, we will see
how to define conversions for custom types.
