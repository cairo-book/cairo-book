...
The following list contains keywords that are reserved for current or future use
by the Cairo language.
...
any items to follow good practices.
...
- `implicits` - Special kind of function parameters that are required to perform
  certain actions
- `let` - Bind a variable
...
These keywords are associated with a specific behaviour, but can also be used to
define items.
...
These keywords aren't used yet, but they are reserved for future use. For now,
...
The Cairo programming language provides several specific functions that serve a
special purpose. We will not cover all of them in this book, but using the names
of these functions as names of other items is not recommended.

- `assert` - This function checks a boolean expression, and if it evaluates to
  false, it triggers the panic function.
- `panic` - This function acknowledges the occurrence of an error and terminates
  the program.
