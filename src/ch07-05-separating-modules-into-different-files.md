...
For example, let’s start from the code in Listing {{#ref use-keyword}} that had
...
shown in Listing {{#ref front-extraction}}. Note that this won’t compile until
we create the _src/front_of_house.cairo_ file.
...
{{#label front-extraction}} <span class="caption">Listing
{{#ref front-extraction}}: Declaring the `front_of_house` module whose body will
be in _src/front_of_house.cairo_</span>

Next, place the code that was in the curly brackets into a new file named
_src/front_of_house.cairo_, as shown in Listing {{#ref module-foh}}. The
...
{{#label module-foh}} <span class="caption">Listing {{#ref module-foh}}:
Definitions inside the `front_of_house` module in
_src/front_of_house.cairo_</span>
...
to an Item in the Module Tree"][path] chapter. In other words, `mod` is _not_ an
...
Note that the `use crate::front_of_house::hosting;` statement in _src/lib.cairo_
