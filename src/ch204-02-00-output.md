...
The output segment is a contiguous block of cells starting at a base address.
...
The Output Builtin’s integration with public memory is required for STARK proof
construction and verification:

1. **Public Commitment**: Values written to `output_ptr` become part of the
   public memory, committing the program to those outputs in the proof.
2. **Proof Structure**: The output segment is included in the public input of a
   trace, with its boundaries tracked (e.g., `begin_addr` and `stop_ptr`) for
   verification.
...
These implementation references of the Output Builtin in various Cairo VM
implementations:
