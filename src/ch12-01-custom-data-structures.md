...
[dictionary type](./ch03-02-dictionaries.md) called `Felt252Dict<T>` that allows
...
> Note: Several concepts used in this chapter were already presented earlier in
...
custom _user database_ that will allow us to add users and query them. We will
...
[Generic types](./ch08-00-generic-types-and-traits.md) we also need to correctly
establish the requirements of `T` so it can be a valid `Felt252Dict<T>` value
type:

1. `T` should implement the `Copy<T>` since it's required for getting values
   from a `Felt252Dict<T>`.
2. All value types of a dictionary implement the `Felt252DictValue<T>`, our
   generic type should do as well.
3. To insert values, `Felt252DictTrait<T>` requires all value types to be
...
> Note: The core library of Cairo already includes a `Vec<T>` data structure,
...
[Alexandria](https://github.com/keep-starknet-strange/alexandria/blob/main/packages/data_structures/src/vec.cairo).
...
[Alexandria][alexandria data structures] library, in the "data_structures"
crate.

[alexandria data structures]:
  https://github.com/keep-starknet-strange/alexandria/tree/main/packages/data_structures/src
