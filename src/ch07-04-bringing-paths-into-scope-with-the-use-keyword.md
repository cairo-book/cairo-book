...
In Listing {{#ref use-keyword}}, we bring the `crate::front_of_house::hosting`
...
{{#label use-keyword}} <span class="caption">Listing {{#ref use-keyword}}:
Bringing a module into scope with `use`</span>
...
In Listing {{#ref use-keyword}}, you might have wondered why we specified
...
{{#label unidiomatic-use}} <span class="caption">Listing
{{#ref unidiomatic-use}}: Bringing the `add_to_waitlist` function into scope
with `use`, which is unidiomatic</span>

Although both Listing {{#ref use-keyword}} and {{#ref unidiomatic-use}}
accomplish the same task, Listing {{#ref use-keyword}} is the idiomatic way to
...
of the full path. The code in Listing {{#ref unidiomatic-use}} is unclear as to
where `add_to_waitlist` is defined.

On the other hand, when bringing in structs, enums, traits, and other items with
`use`, it’s idiomatic to specify the full path. Listing {{#ref idiomatic-use}}
...
{{#label idiomatic-use}} <span class="caption">Listing {{#ref idiomatic-use}}:
Bringing `BitSize` trait into scope in an idiomatic way</span>
...
local name, or _alias_, for the type. Listing {{#ref as-keyword}} shows how you
...
{{#label as-keyword}} <span class="caption">Listing {{#ref as-keyword}}:
Renaming a trait when it’s brought into scope with the `as` keyword</span>
...
{{#label multiple-imports}} <span class="caption">Listing
{{#ref multiple-imports}}: Importing multiple items from the same module</span>
...
{{#label reexporting}} <span class="caption">Listing {{#ref reexporting}}:
...
dedicated `[dependencies]` section in your _Scarb.toml_ configuration file. Note
...
_alexandria_math_ crate from _alexandria_ package:
...
which will automatically edit the _Scarb.toml_ file for you. For development
