...
When writing a program, it is quite common to print some data to the console,
...
Types][byte array]), which can be a simple string to print a message or a string
...
> `print!` and `println!` macros use the `Display` trait under the hood, and are
...
> will get an error. In that case, you can either [manually
> implement][print with display] the `Display` trait for your type or use the
> `Debug` trait (see [below][print with debug]).
...
Custom Data Types][print with display]).
