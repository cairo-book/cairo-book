...
used generics in [Chapter {{#chap enums-and-pattern-matching}}][option enum]
with `Option<T>`.
...
generics can be used to reduce duplication.
...
We've now been tasked with finding the largest number in two different arrays of
numbers. To do so, we can choose to duplicate the previous code and use the same
logic at two different places in the program, as follows:
