...
- The generic impl `Ownable` requires the implementation of the
  ...
  `ComponentState<TContractState>` will allow us to embed `OwnableImpl` in any
...
- `Ownable` is annotated with the `embeddable_as(<name>)` attribute:
  ...
  the `embeddable_as(OwnableImpl)` annotation:
