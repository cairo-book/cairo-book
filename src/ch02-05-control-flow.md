...
of execution of Cairo code are if expressions and loops.
...
explore the `if` expression. In the _src/lib.cairo_ file, input the following:
...
{{#chap enums-and-pattern-matching}}][match] describes a powerful Cairo
...
experiment with loops, let’s create a new project called _loops_.
...
> Note: Cairo prevents us from running program with infinite loops by including
...
> the `--available-gas` flag set to a value that is large enough to run the
> program.
...
In Listing {{#ref while-true}}, we use `while` to loop the program three times,
...
{{#label while-true}} <span class="caption">Listing {{#ref while-true}}: Using a
`while` loop to run code while a condition holds `true`.</span>
...
such as an array. For example, the loop in Listing {{#ref iter-while}} prints
...
{{#label iter-while}} <span class="caption">Listing {{#ref iter-while}}: Looping
through each element of a collection using a `while` loop</span>
...
{{#label iter-for}} <span class="caption">Listing {{#ref iter-for}}: Looping
through each element of a collection using a `for` loop</span>

When we run this code, we’ll see the same output as in Listing
{{#ref iter-while}}. More importantly, we’ve now increased the safety of the
...
Listing {{#ref while-true}}. Another way to run code a certain number of times
...
> Note: For our example, our findings came from understanding the **statements**
...
> [Exploring Sierra](https://medium.com/nethermind-eth/under-the-hood-of-cairo-1-0-exploring-sierra-7f32808421f5).
