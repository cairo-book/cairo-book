...
In this section, we are going to be looking at the different types of functions
you could encounter in Starknet smart contracts.
...
{{#ref reference-contract}} that we'll be using throughout this chapter:

```cairo,noplayground
{{#include ../listings/ch101-building-starknet-smart-contracts/listing_01_reference_contract/src/lib.cairo:all}}
```

{{#label reference-contract}} <span class="caption">Listing
{{#ref reference-contract}}: Our reference contract for this chapter</span>

## 1. Constructors

Constructors are a special type of function that only runs once when deploying a
contract, and can be used to initialize the state of a contract.
...
2. The constructor function must be named `constructor`, and must be annotated
...
> standalone functions annotated with the `#[external(v0)]` or functions within
> an impl block annotated with the `#[abi(embed_v0)]` attribute, **must** take
> `self` as a first argument. This is not the case for private functions.
...
View functions are _public_ functions where the `self: ContractState` argument
...
The compiler will mark their _state_mutability_ to `view`, preventing any state
...
> subcontexts. Hence developers **should not** have the assumption that calling
> a view function on another contract cannot modify the state.
...
> definition for this implementation? Well, the `#[generate_trait]` attribute is
...
entrypoints of a contract (see [next section][abi per item section]).
