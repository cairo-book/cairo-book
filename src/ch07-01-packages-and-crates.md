...
get compiled with the crate, as will be discussed in the subsequent sections.
...
in the ["Defining Modules to Control Scope"][modules] chapter.
...
You can create a new Cairo package using the Scarb command-line tool. To create
a new package, run the following command:

```bash
scarb new my_package
```

This command will generate a new package directory named _my_package_ with the
following structure:
...
- _src/_ is the main directory where all the Cairo source files for the package
  will be stored.
...
[dependencies]
cairo_execute = "2.20.0"
```

As you develop your package, you may want to organize your code into multiple
Cairo source files. You can do this by creating additional _.cairo_ files within
the _src_ directory or its subdirectories.
